...
bool handle_mm_fault(struct vm_entry *vme)
{
  ...
  bool success = install_page(vme->vaddr,new_page->kaddr, vme->writable);
  if(!success)
	{
		free_page(new_page->kaddr);
		return false;
...
bool expand_stack(void *addr)
{
  ...
  struct page *stack_page = alloc_page(PAL_USER);
	if(stack_page == NULL)
	{
		free(vme);
		return false;
	...
	stack_page->vme = vme;
	if(!install_page(vme->vaddr, stack_page->kaddr, vme->writable) || !insert_vme(&thread_current()->vm, vme))
	{
...
process_execute (const char *file_name) 
{
  ...
  tid_t tid;
	...
	strlcpy(copy, file_name, strlen(file_name)+1);
        ...
        strlcpy(parsed, argv[0], strlen(argv[0])+1);
        if(filesys_open(parsed) == NULL) return -1;
  ...
  sema_down(&thread_current()->sema_L);
  if (tid == TID_ERROR){
...
{ 
	...
	for(elem=list_begin(&thread_current()->child); elem!=list_end(&thread_current()->child); elem=list_next(elem)){
		...
		if (child_tid == child->tid){
			sema_down(&child->sema_W);
			list_remove(&child->child_elem);
			exit_status = child->exit_status;
			sema_up(&child->sema_E);
			return exit_status;
		}		
	}
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  if (!setup_stack (esp))
	...
	void* flag = *esp;
        ...
        *esp-=4; **(uint32_t**)esp=0;
        for(int i=argc-1;i>=0;i--){
		flag -=(strlen(argv[i])+1);
                *esp-=4;
                **(uint32_t**)esp=(uint32_t)flag;
        }
  ...
  *eip = (void (*) (void)) ehdr.e_entry;
  ...
  file_close (file);
  sema_up(&thread_current()->parent->sema_L);
  return success;
}
/* load() helpers. */
